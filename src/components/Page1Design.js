import React from "react";

function Page1Design() {


    return (

        <div className="pageheight" id="page1">
            <div className="pageControl" />
            <div className="page1text">
                <h4>
                    With 3 years of experience in software development and 5 years of effective managerial leadership, I have a proven track record of engaging with diverse customers and training team members collaboratively.
                </h4>

            </div>
            <div className="page2text">
                <h4>
                    I am bilingual in Korean and English, holding a Bachelor of Fine Arts. My professional background includes proficiency in Ruby on Rails and JavaScript programming, complemented by a solid foundation in Arts and Design.
                </h4>
            </div>
            <div className="page3text">
                <h4>I am adept at using various tools, including Quickbooks, Adobe Photoshop, and Microsoft/Google platforms, and I am well-versed in communication tools like Zoom. Currently, I am actively seeking full-time Full-Stack Developer positions.
                </h4>

            </div>
            <div className="page4text">
                <h4>I am a quick learner and enjoy a face-paced environment.</h4>

            </div>
            <div className="lines1" />
            <div className="lines2" />
            <div className="lines3" />
            <div className="lines4" />
            <div className="lines5" />
            <div className="lines6" />


        </div>

    );

}

export default Page1Design;
